...
sumstat='/gpfs/commons/home/tlin/output/bellenguez/bellenguez_all_2'
missing=$sumstat/finemap_snpvar_constrained/max_snp_1/missing_window.txt
...
do	 
	if  grep -q ".gz" <<< $line ; then
		...
		python finemapper.py \
			--sumstats $sumstat/bellenguez_all.${chr}.snpvar_constrained.gz \
			--n 487511 \
	  		--chr ${chr} --start $start --end $end \
	  		--method susie \
     	  		--max-num-causal 1 \
	  		--allow-missing \
			--out $sumstat/finemap_snpvar_constrained/max_snp_1/finemap_bellenguez_all_2.${chr}.$start.$end.gz
	fi
