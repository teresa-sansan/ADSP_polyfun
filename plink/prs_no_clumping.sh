#!/bin/bash
#SBATCH --job-name=prs_bellenguez_pip
#SBATCH --mail-type=FAIL,END
#SBATCH --mail-user=tlin@nygenome.org
#SBATCH --mem=10G
#SBATCH --time=10:00:00
#SBATCH --array=1-22%7
#SBATCH --output=/gpfs/commons/home/tlin/output/prs/new_anno_0318_24/bellenguez_adsp_reference/thres/plink_output/%x_%j.log
...
do 
  echo running $anno
  zcat $adsp_path/${sumstat}_${anno}_chr${chr}.txt.gz > $output/${sumstat}_${anno}_chr${chr}.txt
  if [ "$anno" == "susie" ]; then 
    SNP=1
    beta_mean=11
  else 
  ...
  --score  $output/${sumstat}_${anno}_chr${chr}.txt $SNP 5 $beta_mean header \
