...
#do
	...
	cd /gpfs/commons/home/tlin/output/cT/new_plink_genomewide/wightman/fixed_beta/ADSP   
	pwd
	for i in e-5 0.001 0.005 0.01 0.05 0.1 0.5
	do
	echo write pT_$i.prs
	awk '{ sum[$2]+=$6 } END {for (user in sum) print user, sum[user] }' ADSP_pT_*.$i.profile > pT_${i}.prs
	done
	echo 
 #done
#done
