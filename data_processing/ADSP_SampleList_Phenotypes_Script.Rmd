...
#  Define the rules to remove duplicates based on the observations fin non-concordant phenotypes

## WHY DONT REMOVE NAs DIRECTLY!!!!!!
...
## shouldnt this be the opposite??
    ...
    if (!is.na(v1$AGE_harmonized) & !is.na(v2$AGE_harmonized)) {
        if (v1$AGE_harmonized>=v2$AGE_harmonized) { select="Select1"; } else { select="Select2"; } ## why deal with the same age this way?? 
    }
...
for (i in 1:nrow(duplist.unknown)) {
    ...
    rowidx1=which(d7$SUBJID==subj1) ## choice 1
    rowidx2=which(d7$SUBJID==subj2) ## choice 2
    rec1=d7[rowidx1,]
    rec2=d7[rowidx2,]
    
    if (nrow(rec1)==0) {duplist.unknown$Choice[i]="PrunedEarlier1"} else {
    if (nrow(rec2)==0) {duplist.unknown$Choice[i]="PrunedEarlier2"} else {
        r=resolve(rec1,rec2) ## decide which one to choice
...
```
```{r}
#concordant <- read.table('/gpfs/commons/groups/knowles_lab/data/ADSP_reguloML/ADSP_vcf/phenotype_file/release_36K/pheno_concordant.tsv', header=T, as.is=T,sep="\t",stringsAsFactors=FALSE,comment="",quote="")

#pheno_SampleID <- read.table('/gpfs/commons/groups/knowles_lab/data/ADSP_reguloML/ADSP_vcf/phenotype_file/release_36K/pheno_SampleID.tsv', header=T, as.is=T,sep="\t",stringsAsFactors=FALSE,comment="",quote="")
```
