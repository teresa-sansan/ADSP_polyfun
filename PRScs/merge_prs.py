...
prs_0 = pd.read_csv(path+name+"_0.tsv", sep = ' ', names= ["IID","SCORE"])
...
prs = [ prs_e5 ,prs_e4,prs_001, prs_01 , prs_1,prs_9, prs_0]  
prs_merge = reduce(lambda left, right:pd.merge(left,right,on=["IID"]),prs)
prs_merge = prs_merge.set_axis( ["IID","PRS_e5","PRS_e4","PRS_001","PRS_01","PRS_1","PRS_9","PRS_0"], axis='columns')
