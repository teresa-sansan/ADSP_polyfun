...
def splash_screen():
    ...
    print('# Run susieRSS')
    print('# Edited by Teresa Lin (July 2024)')
    print('*********************************************************************')
    print()
...
def read_ld_from_file(ld_file):
    ...
    if not ld_file.endswith('.bcor') and not ld_file.endswith('.npz'):
        ...
        elif os.path.exists(ld_file+'.ld'):
            ld_file = ld_file + '.ld'
        else:
...
class SUSIE_Wrapper(Fine_Mapping):
    ...
    def finemap(self, locus_start, locus_end, num_causal_snps, use_prior_causal_prob=True, prior_var=None, residual_var=None, residual_var_init=None, hess_resvar=False, hess=False, hess_iter=100, hess_min_h2=None, susie_max_iter=100, verbose=False, ld_file=None, debug_dir=None, allow_missing=False, susie_outfile=None, finemap_dir=None):
        ...
        if use_prior_causal_prob:
            ...
            print('## There are %d SNPs that have negative SNPVAR, and they are now set to be 1e-10'%(np.sum(prior_weights < 0)))
            prior_weights[prior_weights <=0] = 1e-10
            try:
                assert np.isclose(prior_weights.sum(), 1)

            except:
                print('## [Warning] Weight sum is not 1, instead, it is %.4f'% prior_weights.sum())
                print( np.sum(prior_weights >1) )
                assert np.sum(prior_weights <0) == 0
        ...
        Z = self.df_sumstats_locus['Z'].values.copy()
        ...
        try:
            ...
            negative = np.sum(beta_var < 0)
            print('## %d of beta_var is < 0, with the smallest beta_var as %.6f'%(negative, np.min(beta_var)))
            beta_var[beta_var <= 0] = 1e10
