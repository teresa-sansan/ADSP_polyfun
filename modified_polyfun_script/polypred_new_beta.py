## We are useing beta from the newly calculated beta instead of the original beta mean
...
def load_betas_files(betas_file, verbose=True):
    ...
    ## remove BETA_MEAN if needed.
    if df_betas.columns.str.contains('NEW_BETA').any():
        print("remove BETA_MEAN")
        df_betas = df_betas.drop(columns= ['BETA_MEAN'])
        print(df_betas.columns)

    #rename columns if needed
    df_betas.rename(columns={'sid':'SNP', 'nt1':'A1', 'nt2':'A2', 'BETA_MEAN':'BETA', 'NEW_BETA':'BETA', 'ldpred_inf_beta':'BETA', 'chrom':'CHR', 'Chrom':'CHR', 'pos':'BP'}, inplace=True, errors='ignore')
