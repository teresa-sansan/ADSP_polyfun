...
do
    ...
    do
        ~/plink \
            ...
            --out $dir_beta/prs/middle_file/${flag}${anno}_pip${thres}.chr${chr}
    done
    awk '{ sum[$2]+=$6 } END {for (user in sum) print user, sum[user] }' $dir_beta/prs/middle_file/${flag}${anno}_pip${thres}.chr*.profile > $dir_beta/prs/middle_file/${flag}${anno}_pip_${thres}.prs
...
python merge_diagnosis_toplot.py $anno /gpfs/commons/groups/knowles_lab/data/ADSP_reguloML/summary_stats/alzheimers/ADSP_reference_panel/fine_mapping/annotations_dl/finemap_v3_backup_teresa/prs/ $flag
