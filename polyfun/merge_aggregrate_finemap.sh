...
do 
  ...
  zcat aggregrate.all.txt.gz| head -1 > agg_bellenguez_extract_1e-3.tsv
  #zcat aggregrate.all.txt.gz| awk '{if($9 <= 0.001) print$0}' >> agg_bellenguez_extract_1e-3.tsv
  zcat aggregrate.all.txt.gz| awk '{if($10 <= 0.001) print$0}' >> agg_bellenguez_extract_1e-3.tsv
  echo finished, total line = $( cat agg_bellenguez_extract_1e-3.tsv | wc -l )
done
